...
public class AndroidSocketsRecieverForDragAndDropSetViewV7 extends Activity {
	private Boolean exit = false;
	private Boolean isDragging = false;

	private List<View> viewsList;
	// private View movingView = null;
	private AndroidSocketsRecieverForDragAndDropSetViewV7 a = this;
	...
	private OnTouchListener mOnTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View view, MotionEvent motionEvent) {
			...
			LinearLayout topleftGroupView = (LinearLayout) topleft;
			LinearLayout toprightGroupView = (LinearLayout) topright;
			...
			case MotionEvent.ACTION_DOWN:
				...
				if (firstOuterLayout.getId() == R.id.topleft || firstOuterLayout.getId() == R.id.topright) {
					...
					//firstOuterLayout.removeViewInLayout(view);
...
//				if (owner3 != null && (owner3.getId() == R.id.topleft || owner3.getId() == R.id.topright)) {
//					owner3.removeView(view);
//				}
				float curX = motionEvent.getX();
				float curY = motionEvent.getY();
//				Log.d("Coord", "X:" + String.valueOf(curX) + ", " + "Y:" + String.valueOf(curY));
//				View underView = ViewTraversal.getView(curX, curY, viewsList);
				if(ViewTraversal.isPointInsideView(curX, curY, topleft)){
					rootView.removeView(view);
					//rootView.removeViewInLayout(view);
					view.setX(topleft.getX());
					view.setY(topleft.getY());
					topleftGroupView.addView(view);
					topleftGroupView.bringChildToFront(view);
				}else if(ViewTraversal.isPointInsideView(curX, curY, topright)){
					rootView.removeView(view);
					//rootView.removeViewInLayout(view);
					view.setX(topright.getX());
					view.setY(topright.getY());
					toprightGroupView.addView(view);
					toprightGroupView.bringChildToFront(view);
				}else{
					rootView.removeView(view);
					//rootView.removeViewInLayout(view);
					view.setX(topleft.getX());
				...
				view.requestLayout();
//				LinearLayout underViewToGroupView = (LinearLayout) underView;
//				if (underViewToGroupView != null) {
//					view.setX(underViewToGroupView.getX());
//					view.setY(underViewToGroupView.getY());
//			
//					underViewToGroupView.addView(view);
//					underViewToGroupView.bringChildToFront(view);
//				} else {
//					rootView.removeView(view);
//					View ax = findViewById(R.id.topright);
//					LinearLayout sd =  (LinearLayout) ax;
//					sd.addView(view);
//					view.setX(sd.getX());
//					view.setY(sd.getY());
//					sd.bringChildToFront(view);
//				}
				return true;
			default:
				break;
